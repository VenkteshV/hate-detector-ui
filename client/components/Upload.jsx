...
class Upload extends Component {
  ...
  sendRequest(file, isExpand) {
    return new Promise((resolve, reject) => {
      ...
      req.open("POST", "http://localhost:5000/detect/hate");
      req.send(formData);
    });
  }
  ...
  render() {
    ...
    console.log("bloomtaxonomy", this.state.result);
    return (
      <div>
        ...
        <div className="Card">
...

         {
          !_.isEmpty(this.state.result) ?
            (<div> <span className="Title">{JSON.parse(this.state.result).result}</span></div>) : (null)
        }
      </div>
